...
var SYSTEM_PROMPT = `You are the AI assistant for Creative Drywall, Montana's premier family-owned drywall company since 1976. The Thompson family has provided exceptional drywall services for 49+ years across 4 generations.
  ...
  async fetch(request, env2) {
    ...
    try {
      ...
      if (!sanitizeResult.safe) {
        return new Response(
          JSON.stringify({ success: false, message: sanitizeResult.reason }),
          { status: 400, headers: corsHeaders }
        );
      }
      ...
      if (isOffTopic(userMessage)) {
        return new Response(JSON.stringify({
          success: true,
          message: `I'm here for drywall questions! ${BUSINESS_INFO.heritage}. How can I help with your project? \u{1F4DE} ${BUSINESS_INFO.phone}`
...
\u{1F4DE} Call: ${BUSINESS_INFO.phone}
\u{1F4E7} Email: ${BUSINESS_INFO.email}
\u{1F4CD} ${BUSINESS_INFO.address}`
        }), { status: 200, headers: corsHeaders });
      }
...
\u{1F6A8} **Call ${BUSINESS_INFO.phone} immediately.**
...
${BUSINESS_INFO.heritage} - we've handled countless emergencies.`
      ...
      let aiResponse;
      try {
        const response = await env2.AI.run("@cf/meta/llama-3-8b-instruct", {
          messages,
          max_tokens: 512,
          temperature: 0.7
        });
        ...
        return new Response(
          JSON.stringify({ success: true, message: getFallback() }),
          { status: 200, headers: corsHeaders }
        );
      }
      return new Response(JSON.stringify({
...
// .wrangler/tmp/bundle-QGUQyp/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QGUQyp/middleware-loader.entry.ts
